...
function calculateFormPrices() {
    ...
    // Dalali (Disabled for Sales)
    const dalaliPct = 0;
    const dalaliAmount = 0;
...
function setupRealTimeValidation() {
    ...
    // Validate duplicate Selling Number
    ...
    sellingNoInput.addEventListener("input", () => {
        ...
        if (salesList.some(sale => sale.sellingNo === no)) {
...
function validateForm() {
    ...
    if (isValid && initialReceived > 0) {
        ...
        if (initialPaymentType === "Bill" && initialReceived > (netBillAmount + 0.01)) {
            ...
            showFieldError(initialReceivedInput, `Received amount exceeds the party outstanding of ${formatCurrency(finalAmount)}.`);
            isValid = false;
        }
    }
